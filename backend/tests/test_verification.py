...
with TestClient(app) as client:
    ...
    print(f"_rawnet2 is None: {main_mod._rawnet2 is None}")
    print(f"_ecapa is None:   {main_mod._ecapa is None}")
    h = client.get("/health").json()
    print(f"Health check response: {h}")

    print("\n=== QUESTION 1: SYNTHETIC VS GENUINE COMPARISON (REAL WEIGHTS) ===")
